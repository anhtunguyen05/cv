...
- [x] TASK-1-2-02: Deliver secure session lifecycle backend
  - Status: `done`
  - Owner: `Codex`
  - Branch/worktree: `feat/task-1-2-02-auth-session-backend`
  ...
  - Outcome: 01. Implement login and logout application/API behavior: Implemented login and logout application/API behavior. | 02. Enforce login privacy and abuse controls: Enforced login privacy and abuse controls. | 03. Verify backend auth lifecycle: Verified backend auth lifecycle.
  ...
  - Verification: 01. Implement login and logout application/API behavior: PHPUnit and Laravel feature tests passed. | 02. Enforce login privacy and abuse controls: generic failure, IP/email throttle, CSRF, bearer, expiry and redaction feature tests passed. | 03. Verify backend auth lifecycle: focused suite `8 tests, 59 assertions` and full API suite `23 tests, 120 assertions` passed against PostgreSQL 16 and Redis.
