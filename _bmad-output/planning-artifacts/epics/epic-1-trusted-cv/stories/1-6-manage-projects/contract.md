# Story 1.6: Manage projects — Contract
...
| --- | --- | --- | --- | --- |
| Add project | Profile project write operation selected by `E1-DEC-004` | Approved project fields and concurrency token when required | Updated shared Profile resource with server item ID | Nested validation, stale conflict, auth, and hidden Profile |
| Edit/remove project | Same approved Profile project boundary | Stable server project ID plus approved changes | Updated shared Profile resource | Invalid/foreign item, stale conflict, auth, and hidden Profile |

Project shape, links, limits, ordering, and nested errors must reuse `E1-CONTRACT-PROFILE-001`; `E1-DEC-003` through `E1-DEC-005` freeze the remaining choices.
