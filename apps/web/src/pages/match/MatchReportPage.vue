...
import { ArrowLeft, ArrowRight, Sparkles, CheckCircle2 } from 'lucide-vue-next'
  ...
  matched_skills_json: [
    {
      ...
      evidence: 'CareerFitCV & Edura projects',
    },
    {
      skill: 'TypeScript & Typings',
      status: 'matched' as const,
      evidence: 'All recent repository work',
    },
    {
      skill: 'RESTful API & HTTP',
      status: 'matched' as const,
      evidence: 'Laravel backend integration',
    },
  ...
  weak_sections_json: [
    {
      ...
      evidence: 'Only listed in tools list, missing project bullets',
    },
  ],
  recommendations_json: [
    {
      section: 'Projects',
      recommendation:
        'Add a bullet in CareerFitCV highlighting Pinia state architecture — it directly addresses the JD keyword requirements.',
    ...
    },
    {
      section: 'Skills',
      recommendation:
        'Include Vitest or unit test experience in technical skills section to satisfy the testing requirement.',
      priority: 'high' as const,
      impact: '+5 pts',
    },
    {
      section: 'Summary',
      recommendation:
        'Mention "TypeScript" and "RESTful API" within your professional summary for higher front-loaded keyword frequency.',
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-0.5">
        <div>
          <div class="flex items-center gap-2 mb-1.5">
            ...
            <span
              class="text-xs font-semibold text-text-quiet bg-surface-muted px-2.5 py-0.5 rounded-full border border-border"
              >{{ stubReport.company_name }}</span
            >
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
      ...
      <div class="lg:col-span-4 space-y-5">
        ...
        <Card class="bg-gradient-to-b from-white to-surface space-y-4">
          ...
          <div
            class="p-3 rounded-xl bg-success-muted border border-success-border flex items-center justify-center gap-2 text-xs sm:text-sm text-success-text"
          >
        ...
        <Card variant="dark" class="space-y-3.5">
          ...
          <RouterLink to="/cv/1/edit" class="block pt-1">
            <AppButton size="sm" variant="glass" class="w-full"> Open CV Editor </AppButton>
          </RouterLink>
        </Card>
      </div>
      ...
      <div class="lg:col-span-8 space-y-6">
        ...
        <Card class="space-y-4">
          <div>
            <h2 class="text-lg sm:text-xl font-bold text-text tracking-tight">
              Keyword Coverage & Evidence
            </h2>
            <p class="text-xs sm:text-sm text-text-muted mt-0.5 leading-relaxed">
              Audited cross-reference between your project bullets and the target job description.
            </p>
          </div>
        ...
        <Card class="space-y-4">
          <div>
            ...
            <p class="text-xs sm:text-sm text-text-muted mt-0.5 leading-relaxed">
              Evidence-backed suggestions to close missing keyword gaps.
            </p>
          </div>
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <span class="text-xs sm:text-sm font-bold text-text uppercase tracking-wider"
                    >{{ rec.section }} Section</span
                  >
                  <AppBadge
                    :variant="priorityColors[rec.priority].variant"
                    :label="priorityColors[rec.priority].label"
                    size="sm"
                  />
                </div>
                <span
                  class="text-xs font-mono font-bold text-success-hover bg-success-muted px-2.5 py-0.5 rounded-full border border-success-border"
                >
                  {{ rec.impact }}
                </span>
              </div>
